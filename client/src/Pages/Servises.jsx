    ...
    <section id="services">
      <div className="services-container">
        <div className="contents">
          ...
          <div
            className="service boxb"
            onClick={() => setShowAdvancedCourseModal(true)}
          >
            <h2>Apply Advanced Courses</h2>
            {/* <p>Get personalized assistance from expert tutors in real-time through live sessions.</p> */}
          </div>
          <div
            className="service boxc"
            onClick={() => setShowAppealFormModal(true)}
          >
            <h2>Here Form of Appeal Forms</h2>
            {/* <p>Access comprehensive study materials including notes, quizzes, and practice exams.</p> */}
          </div>
          <div
            className="service boxd"
            onClick={() => setShowClubFormModal(true)}
          >
      ...
      {showScholarshipModal && (
        <ScholarshipFormModal onClose={handleCloseScholarshipFormModal} />
      )}
      {showAdvancedCourseModal && (
        <AdvancedCourseFormModal
          onClose={() => setShowAdvancedCourseModal(false)}
        />
      )}
      {showAppealFormModal && (
        <AppealFormModal onClose={() => setShowAppealFormModal(false)} />
      )}
      {showClubFormModal && (
        <ClubFormModal onClose={() => setShowClubFormModal(false)} />
      )}
    </section>
  );
};
